...
def generate_strength_training_day4(goal):
  """
  Generates a randomized strength training workout plan for day 4.

  Args:
      goal (str, optional): The primary goal of the workout (e.g., weight loss, muscle building).

  Returns:
      str: The workout plan string containing exercises, sets, and reps.
  """
  ...
  # Day 4 focus on Push & Core
  workout = random.sample(exercises[:4] + exercises[-3:], 5)  # Combine push and core
  workout_plan = "Day 4 Workout:\n"
  for exercise in workout:
...
def generate_strength_training_day5(goal):
  """
  Generates a randomized strength training workout plan for day 5.
  ...
  # Day 5 focus on Pull & Core
  workout = random.sample(exercises[4:7] + exercises[-3:], 5)  # Combine pull and core
  workout_plan = "Day 5 Workout:\n"
  for exercise in workout:
    sets = random.randint(3, 4)
    reps = random.randint(8, 12)
    workout_plan += f"- {exercise} ({sets} sets of {reps} reps)\n"
...
def generate_strength_training_day6(goal):
  """
  Generates a randomized strength training workout plan for day 6.
  ...
  # Day 6: Lighter weights/higher reps (optional)
  workout = random.sample(exercises, 4)  # Reduce workout volume
  workout_plan = "Day 6 Workout:\n"
  for exercise in workout:
    sets = random.randint(3, 4)
    reps = random.randint(12, 15)  # Adjust rep range for higher reps
    workout_plan += f"- {exercise} ({sets} sets of {reps} reps)\n"

  return workout_plan
def generate_strength_training_day7(goal):
  """
  Generates a message for Day 7 (rest or active recovery).

  Args:
      goal (str, optional): The primary goal of the workout (ignored for day 7).

  Returns:
      str: A message encouraging rest or active recovery.
  """

  workout_plan = "Day 7 Rest or Active Recovery\n"
  workout_plan += "  - Rest or perform light active recovery activities (e.g., walking, yoga) to promote recovery.\n"
  workout_plan += "  - Consider incorporating activities like:\n"
  workout_plan += "  - Stretching or mobility work\n"
  workout_plan += "  - Low-impact cardio (e.g., swimming, cycling)\n"
  workout_plan += "  - Sports or hobbies you enjoy\n"
...
def generate_workout(goal, day):
  ...
  if day == 1:
      ...
      workout_plan += generate_strength_training_day4(goal) + "\n"
  elif day == 5:
      workout_plan += generate_strength_training_day5(goal) + "\n"
  elif day == 6:
      workout_plan += generate_strength_training_day6(goal) + "\n"
  elif day == 7:
      workout_plan += generate_strength_training_day7(goal) + "\n"
